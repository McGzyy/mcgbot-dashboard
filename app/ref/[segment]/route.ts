...
export async function GET(
  ...
  try {
    ...
    if (!r.ok) {
      return NextResponse.redirect(new URL("/subscribe", apiBase));
    }
    const j = (await r.json().catch(() => null)) as
      | { ok?: boolean; discord_id?: string }
      | null;
    const id = typeof j?.discord_id === "string" ? j.discord_id.trim() : "";
    if (!id) return NextResponse.redirect(new URL("/subscribe", apiBase));

    const url = new URL(`/subscribe?ref=${encodeURIComponent(id)}`, apiBase);
