...
export function MembershipSolCheckout({
  ...
  const payWithSol = useCallback(
    async (opts: { testSol?: boolean }) => {
      setErr(null);
      setQuote(null);
      if (!selectedPlanSlug && !opts.testSol) {
      ...
      try {
        const res = await fetch("/api/subscription/sol/start", {
          ...
          body: JSON.stringify(
            opts.testSol ? { testSol: true } : { planSlug: selectedPlanSlug }
          ),
        });
    ...
    [connection, onActivated, openWalletModal, publicKey, selectedPlanSlug, sendTransaction]
  );
  ...
  return (
    <div className="rounded-2xl border border-violet-500/25 bg-violet-500/[0.06] p-4">
      ...
      <p className="mt-2 text-sm leading-relaxed text-zinc-200">
        Pay with SOL from your wallet.         The amount is quoted from USD using live rates (rounded up to the nearest lamport so the USD quote is
        always met).{" "}
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        <button
          ...
          onClick={() => void payWithSol({})}
          className="h-11 flex-1 rounded-2xl border border-violet-400/40 bg-violet-500/20 px-4 text-sm font-semibold text-violet-50 shadow-[0_16px_50px_rgba(139,92,246,0.18)] transition hover:bg-violet-500/30 focus:outline-none focus:ring-2 focus:ring-violet-400/35 disabled:cursor-not-allowed disabled:opacity-45"
        >
        ...
        {testSolEnabled ? (
          <button
            ...
            onClick={() => void payWithSol({ testSol: true })}
            className="h-11 flex-1 rounded-2xl border border-zinc-600/60 bg-zinc-900/50 px-4 text-sm font-semibold text-zinc-100 transition hover:bg-zinc-800/60 focus:outline-none focus:ring-2 focus:ring-zinc-500/30 disabled:cursor-not-allowed disabled:opacity-45"
          >
            {busy ? "Working…" : "$1 SOL test (1 month)"}
          </button>
        ) : null}
      </div>
      {!publicKey ? (
