  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setErr(typeof json.error === "string" ? json.error : "Could not load call tape.");
        setRows([]);
        return;
      }
      ...
      setErr("Could not load call tape.");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  if (status !== "authenticated") {
    return (
      <div className="mx-auto max-w-lg px-4 py-12">
        <h1 className="text-xl font-semibold tracking-tight text-zinc-50">Call tape</h1>
