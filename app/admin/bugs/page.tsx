...
export default function AdminBugsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setErr(null);
    fetch(`/api/admin/bugs?${query}`, { credentials: "same-origin" })
      .then(async (res) => ({ ok: res.ok, json: await res.json().catch(() => ({})) }))
      .then(({ ok, json }) => {
        if (cancelled) return;
        if (!ok || !json || json.success !== true) {
          setErr(typeof json?.error === "string" ? json.error : "Failed to load bug reports.");
          setRows([]);
          return;
        }
        const r = Array.isArray(json.rows) ? (json.rows as BugRow[]) : [];
        setRows(r);
        setNotesDraft((prev) => {
          const next = { ...prev };
          for (const row of r) {
  ...
  const updateBug = async (id: string, patch: { status?: string; staffNotes?: string }) => {
    if (savingId) return;
    setSavingId(id);
    setErr(null);
    try {
      ...
      // refresh
      const r = await fetch(`/api/admin/bugs?${query}`, { credentials: "same-origin" })
        .then((x) => x.json().catch(() => ({})));
      if (r && r.success === true && Array.isArray(r.rows)) {
        setRows(r.rows as BugRow[]);
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Bug reports"
        description="Closing a bug sends the reporter a bell inbox notification."
        actions={
                    ...
                    return (
                      <tr key={r.id} className="align-top hover:bg-zinc-900/30">
                        <td className="px-4 py-3">
                          <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${statusBadge(r.status)}`}>
                            {r.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="space-y-1">
                            ...
                            {r.page_url ? (
                              <div className="text-[11px] text-zinc-500">
                                Page:{" "}
                                <a href={r.page_url} target="_blank" rel="noopener noreferrer" className="underline decoration-zinc-700 underline-offset-2 hover:decoration-zinc-500">
                                  {r.page_url}
                                </a>
                              </div>
                            ) : null}
                        ...
                        <td className="px-4 py-3">
                          <textarea
                            ...
                            rows={3}
                            className="w-full min-w-[240px] resize-none rounded-lg border border-zinc-800 bg-black/25 px-3 py-2 text-xs text-zinc-100 outline-none ring-amber-500/20 focus:ring-2"
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="inline-flex flex-wrap justify-end gap-2">
                            <button
                              type="button"
                              onClick={() => updateBug(r.id, { status: "triaged" })}
                              disabled={savingId === r.id}
                            ...
                            <button
                              type="button"
                              onClick={() => updateBug(r.id, { staffNotes: draft })}
                              disabled={savingId === r.id}
                            ...
                            <button
                              type="button"
                              onClick={() => updateBug(r.id, { status: "closed", staffNotes: draft })}
                              disabled={savingId === r.id}
