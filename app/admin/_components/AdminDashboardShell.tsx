...
const NAV: {
...
}[] = [
  {
    ...
    icon: (a) => (
      <svg className={`h-4 w-4 ${a ? "text-violet-300" : "text-zinc-600"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden>
...
function navClass(active: boolean) {
  return [
    ...
    active
      ? "border-violet-500/35 bg-gradient-to-br from-violet-950/50 to-zinc-950/80 text-white shadow-[0_0_24px_-4px_rgba(139,92,246,0.35)]"
            ...
            {NAV.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={navClass(active)}>
                  {active ? (
                    <span
                      className="absolute left-0 top-2 bottom-2 w-0.5 rounded-full bg-violet-400 shadow-[0_0_10px_rgba(167,139,250,0.8)]"
                      aria-hidden
                    />
                  ) : null}
