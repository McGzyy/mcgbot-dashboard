...
/** Same bucket as bug reports; paths prefixed with `fr/` for feature uploads. */
...
export async function POST(request: Request) {
  try {
    ...
    const objectPath = `fr/u/${userId}/${Date.now()}-${id}.${ext}`;
    ...
    if (error || !data?.signedUrl) {
      console.error("[feature/upload-url] signed url:", error);
    ...
    console.error("[feature/upload-url] POST:", e);
