...
export function buildProfileTrackChartView(
  ...
  const total = Math.max(0, input.stats.totalCalls);
...
/** @deprecated Use buildProfileTrackChartView */
export function buildProfileCallStripSeries(
  calls: Parameters<typeof buildProfileTrackChartView>[1]["recentCalls"],
  limit = 20,
  nowMs = Date.now()
) {
  const view = buildProfileTrackChartView(
    "avg_x",
    {
      recentCalls: calls,
      stats: { avgX: 0, winRate: 0, totalCalls: 0 },
      hitRates: { rate2x: null, rate3x: null },
    },
    nowMs
  );
  return view.points.map((p, i) => ({
    ...
    timeMs: 0,
    summary: p.detail ?? "",
    timeLabel: formatJoinedAt(0, nowMs, "compact"),
  }));
}
